...
namespace MissingNumberFinderTests
{
    public class FindMissingNumberTests
    {
        ...
        [InlineData(1, 2, 1)]
        [InlineData(2, 4, 9)]
        public void Throws_On_Invalid_Lists(int value0, int value1, int value2)
        {
            int[] values = new int[] { value0, value1, value2 };           
            Assert.Throws<InvalidOperationException>( () => NumberFinder.FindMissingValue(values));
        }
        ...
        [InlineData(1, 2, 3, 4, 6, 7, 8, 9, 10, 11, 5)] // 10 missing (midpoint)
